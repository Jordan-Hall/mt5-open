...
//!   hours, the dated bar request) and the corrected
...
use mt5_native::cipher::{SessionCipher, startup_decrypt, startup_encrypt};
use mt5_native::compression::{MAX_MESSAGE, inflate_inner, make_compressed_payload};
use mt5_native::crypto::{dotnet_utf16, password_hash};
use mt5_native::depth::{DepthEntry, DepthRecord, decode_depth_record, encode_depth_record};
use mt5_native::frame::{COMPRESSED, FINAL, Frame, FrameParser};
use mt5_native::hexutil;
use mt5_native::history::{
    ByteReader, ColumnGroup, TrailingHours, read_column_group, read_trailing_hour_segment,
};
...
use mt5_native::quotes::{QuoteRow, decode_quotes};
use mt5_native::requests::{
    make_password_change, make_tick_history_request, make_trade_history_request,
    request_descriptor_499,
};
use mt5_native::subscription::{
    bar_month_request, date_token, make_depth_subscription_payload, make_subscription_payload,
};
use mt5_native::trade::{
    MarketTradeFields, build_trade_record, make_signed_trade_payload, parse_trade_update_35,
};
...
fn quote_row_to_value(r: &QuoteRow) -> Value {
    ...
    if r.command == 50 {
        ...
        m.insert(
            "compatibility_time_ms".into(),
            num(r.compatibility_time_ms.unwrap()),
        );
    }
...
fn column_group_to_value(g: &ColumnGroup) -> Value {
    let descriptors: Vec<Value> = g
        ...
        .map(|d| {
            ...
            obj.insert(
                "decoded".into(),
                json!({"hex": hexutil::encode(&d.decoded)}),
            );
            obj.insert("compression".into(), json!(d.compression));
            if !d.encoded.is_empty() {
                obj.insert("encoded".into(), json!({"hex":hexutil::encode(&d.encoded)}));
            }
            // Only projected columns carry unused_tail_hex (observed consumer).
            if d.projected {
                obj.insert(
                    "unused_tail_hex".into(),
                    json!(hexutil::encode(&d.unused_tail)),
                );
            }
            Value::Object(obj)
        })
...
fn blob_integrity_and_unique_ids() {
    ...
    for name in ["conformance_vectors.json", "revision3_vectors.json"] {
        ...
        for c in cases(&file) {
            if c["operation"] == "request_contract_only_not_inner_function" {
                continue;
            }
            total_cases += 1;
            assert!(
                ids.insert(c["id"].as_str().unwrap().to_string()),
                "duplicate id"
            );
        }
    }
    assert_eq!(total_cases, 74, "expected 74 protocol fixture cases");
    assert!(
        total_blobs >= 59,
        "expected the documented byte objects, saw {total_blobs}"
    );
}
...
fn record_layouts_are_contiguous() {
    ...
    for (name, record) in records {
        ...
        assert_eq!(
            cursor,
            record["size"].as_i64().unwrap(),
            "{name}: final size"
        );
    }
    assert!(
        field_entries >= 752,
        "expected 752 field entries, saw {field_entries}"
    );
}
...
fn revision3_cases_decode_to_expected() {
    ...
    for case in cases(&file) {
        ...
        if op == "request_contract_only_not_inner_function" {
            continue;
        }
        ...
        let actual: Value = match op {
            ...
            "command102_subtype9_request" => {
                ...
                assert_eq!(
                    bar_month_request(symbol, y, m, d).unwrap(),
                    raw,
                    "{id}: serialized request"
                );
...
fn corrected_command51_snapshots() {
    ...
    for case in cases(&file) {
        ...
        assert_eq!(
            row.symbol_id,
            inp["symbol_id"].as_i64().unwrap() as i128,
            "{id}: symbol"
        );
        ...
        assert_eq!(
            ...
            case["expected"]["record_bits_before_padding"]
                .as_i64()
                .unwrap(),
            "{id}: bit count"
        );
...
fn stateful_session_frames_reproduce() {
    ...
    for seg in expected["segments"].as_array().unwrap() {
        ...
        assert_eq!(
            ...
            seg["ciphertext_hex"].as_str().unwrap()
        );
        let frame = Frame::new(command, sequence, 2, ciphertext);
        assert_eq!(
            hexutil::encode(&frame.pack()),
            seg["frame_hex"].as_str().unwrap()
        );
    }
    assert_eq!(
        cipher.position as u64,
        expected["final_position"].as_u64().unwrap()
    );
    assert_eq!(
        cipher.previous_plain as u64,
        expected["final_previous_plain"].as_u64().unwrap()
    );
}
    ...
    for case in cases(&file) {
        ...
        match op {
            "startup_transform" => {
                ...
                assert_eq!(
                    hexutil::encode(&ct),
                    exp["ciphertext_hex"].as_str().unwrap(),
                    "{id}"
                );
            ...
            "password_truncation_and_hash" => {
                ...
                assert_eq!(
                    hexutil::encode(&password_hash(login, pw)),
                    exp["credential_digest_hex"].as_str().unwrap(),
                    "{id}"
                );
                assert_eq!(
                    hexutil::encode(&dotnet_utf16(pw, Some(16))),
                    exp["encoded_password_hex"].as_str().unwrap(),
                    "{id}"
                );
            }
            "session_key_derivation" => {
                let digest: [u8; 16] =
                    hexutil::decode(inp["credential_digest_hex"].as_str().unwrap())
                        .try_into()
                        .unwrap();
                let tag7 = hexutil::decode(inp["tag7_hex"].as_str().unwrap());
                assert_eq!(
                    hexutil::encode(&session_key_padded_input(&tag7)),
                    exp["zero_padded_input_hex"].as_str().unwrap(),
                    "{id}"
                );
                assert_eq!(
                    hexutil::encode(&derive_session_key_from_digest(&digest, &tag7).unwrap()),
                    exp["session_key_hex"].as_str().unwrap(),
                    "{id}"
                );
            }
            "outer_compression_literal_only" => {
                let pt = hexutil::decode(inp["plaintext_hex"].as_str().unwrap());
                assert_eq!(
                    hexutil::encode(&make_compressed_payload(&pt).unwrap()),
                    exp["envelope"]["hex"].as_str().unwrap(),
                    "{id}"
                );
            }
            "subscription_body" => {
                let ids: Vec<i64> = inp["symbol_ids"]
                    .as_array()
                    .unwrap()
                    .iter()
                ...
                let body = match inp["kind"].as_str().unwrap() {
                    "quotes" => make_subscription_payload(
                        &ids.iter().map(|&x| x as i32).collect::<Vec<_>>(),
                    ),
                    "depth" => make_depth_subscription_payload(
            ...
            "reject_frame_header" => {
                ...
                assert!(
                    FrameParser::new(max).feed(&frame).is_err(),
                    "{id}: expected reject"
                );
            }
            "history_block_inflation" => {
                ...
                assert_eq!(
                    ...
                    exp["plaintext"]["hex"].as_str().unwrap(),
                    "{id}"
                );
            ...
            "packed_integer_sequence" => {
                ...
                let values: Vec<i128> = inp["values"]
                    .as_array()
                    .unwrap()
                    .iter()
                ...
                if !signed {
                    ...
                    // packed-integer encoder (units follow the value's bit-length, so the
                    ...
                    assert_eq!(
                        hexutil::encode(&w.data),
                        exp["bytes"]["hex"].as_str().unwrap(),
                        "{id}: bytes"
                    );
                    assert_eq!(
                        w.position,
                        exp["bit_count"].as_u64().unwrap() as usize,
                        "{id}: bit_count"
                    );
                }
            ...
            "command101_request" => {
                assert_eq!(
                    inp["cache_count"].as_u64(),
                    Some(0),
                    "{id}: only no-cache modelled"
                );
            ...
            "command105_subtype14_request" => {
                ...
                assert_eq!(
                    token as u64,
                    exp["date_token"].as_u64().unwrap(),
                    "{id}: date_token"
                );
                assert_eq!(
                    hexutil::encode(&request_descriptor_499(symbol, token)),
                    exp["descriptor"]["hex"].as_str().unwrap(),
                    "{id}: descriptor"
                );
                let body = make_tick_history_request(
                    symbol,
                    y,
                    m,
                    dd,
                    inp["request_parameter"].as_u64().unwrap() as u32,
                )
                .unwrap();
                assert_eq!(
                    hexutil::encode(&body),
            ...
            "command55_subtype35" => {
                ...
                assert_eq!(
                    u.stride,
                    exp["stride"].as_u64().unwrap() as usize,
                    "{id}: stride"
                );
                // Record offsets in the body: subtype(1)+count(4)=5, then fixed spans.
                assert_eq!(
                    5usize,
                    exp["transaction_offset_in_body"].as_u64().unwrap() as usize,
                    "{id}: txn off"
                );
                assert_eq!(
                    5 + 152,
                    exp["request_offset_in_body"].as_u64().unwrap() as usize,
                    "{id}: req off"
                );
                assert_eq!(
                    5 + 152 + 800,
                    exp["result_offset_in_body"].as_u64().unwrap() as usize,
                    "{id}: res off"
                );
            ...
            "login_value_wrapper_only" => {
                let challenge: [u8; 16] = hexutil::decode(inp["challenge_hex"].as_str().unwrap())
                    .try_into()
                    .unwrap();
                ...
                assert_eq!(
                    v.login_id.to_string(),
                    exp["login_id"].as_str().unwrap(),
                    "{id}: login_id"
                );
                assert_eq!(
                    v.extended_login_id.to_string(),
                    exp["extended_login_id"].as_str().unwrap(),
                    "{id}: ext"
                );
                assert_eq!(
                    hexutil::encode(&v.tag88_value),
                    exp["tag88_value_hex"].as_str().unwrap(),
                    "{id}: tag88"
                );
                assert_eq!(
                    hexutil::encode(&v.tag134_value),
                    exp["tag134_value_hex"].as_str().unwrap(),
                    "{id}: tag134"
                );
            }
            "command52_single_record" => {
                ...
                assert_eq!(
                    encoded,
                    hex_of(exp, "body"),
                    "{id}: signed-magnitude wire fixture"
                );
                assert_eq!(
                    decode_depth_record(&encoded).unwrap().0,
                    rec,
                    "{id}: round-trip"
                );
            }
...
fn signed_trade_full_pipeline() {
    ...
    let case = cases(&file)
        .iter()
        .find(|c| c["operation"] == "signed_trade_full_pipeline")
        .unwrap();
    let inp = &case["inputs"];
    let f = &inp["fields"];
    ...
    assert_eq!(
        hexutil::encode(&record),
        exp["record"]["hex"].as_str().unwrap(),
        "record"
    );

    let trade_key: [u8; 32] = hexutil::decode(inp["trade_key_hex"].as_str().unwrap())
        .try_into()
        .unwrap();
    let plaintext_body = make_signed_trade_payload(&record, &trade_key).unwrap();
    assert_eq!(
        hexutil::encode(&plaintext_body),
        exp["plaintext_body"]["hex"].as_str().unwrap(),
        "plaintext body"
    );
    // The embedded TLV-85 signature must equal the documented HMAC.
    assert_eq!(
        hexutil::encode(&plaintext_body[806..838]),
        exp["hmac_hex"].as_str().unwrap(),
        "hmac"
    );

    let envelope = make_compressed_payload(&plaintext_body).unwrap();
    assert_eq!(
        hexutil::encode(&envelope),
        exp["compression_envelope"]["hex"].as_str().unwrap(),
        "envelope"
    );
