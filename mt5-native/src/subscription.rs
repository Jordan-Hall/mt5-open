//! Quote/depth subscriptions and dated bar-history request codecs.
...
pub fn make_subscription_payload(symbol_ids: &[i32]) -> Vec<u8> {
    ...
    for id in symbol_ids { out.extend_from_slice(&id.to_le_bytes()); }
    out
}
...
pub fn bar_month_request(symbol: &str, year: i32, month: i32, day: i32) -> Result<Vec<u8>> {
    ...
    if field.len() > 64 { return Err(ProtocolError::new("symbol exceeds 64 bytes")); }
    let mut out = vec![0x09u8];
