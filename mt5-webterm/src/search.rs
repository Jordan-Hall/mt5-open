...
fn cookie() -> String {
    ...
    format!(
        "_fz_uniq={val};uniq={val};age={};tid=0",
        timestamp.saturating_sub(86400)
    )
}
...
pub async fn search_mq(company: &str) -> Vec<Value> {
    ...
    let Ok(resp) = client
        ...
        .header(
            "User-Agent",
            "MetaTrader 5 Terminal/5.5830 (Windows NT 10.0.22621; x64)",
        )
    ...
    let Ok(v) = serde_json::from_str::<Value>(&text[idx..]) else {
        return vec![];
    };
    v.get("result")
...
fn hits_named(payload: &[Value], server_name: &str) -> Vec<ServerHit> {
    ...
    for company in payload {
        let Some(results) = company.get("results").and_then(|x| x.as_array()) else {
            continue;
        };
...
pub async fn list_servers(company: &str) -> Result<Vec<String>, String> {
    ...
    let payload = search_mq(company).await;
    for entry in &payload {
        let Some(results) = entry.get("results").and_then(|x| x.as_array()) else {
            continue;
        };
        for result in results {
...
mod tests {
    ...
    #[test]
    fn prefers_443() {
        let access = vec!["acc.example:701".into(), "edge.example:443".into()];
